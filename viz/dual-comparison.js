d3.json("data/annotations.json").then(function(jsonData) {
  ...
  d3.csv("data/evolucao-ibope-limpo.csv").then(function(csvData) {
    ...
    function drawChart(data, target, segment, presidents, mainChart, annotate) {
        ...
        function updateChart(data, measure, presidents, svgSelector, pointSelector, mainChart) {
          ...
          function computeInfo(data, measure, presidents) {
            ...
            function computeClosestPoll(dataArray, bolsoDays, measure) {
              ...
              return {
                date: dataArray[selectedIndex].DATA,
          ...
          function showPoint(pointSelector, highlightDates, mainChart) {
            ...
            if( mainChart ) {
              let points = d3.selectAll(pointSelector)
                  .style("visibility", d => highlightDates.includes(d.DATA) ? "visible" : "hidden" )
              // console.log(points);
            } // End of if
      ...
      function annotateChart(jsonData, target, president, chartScales) {
        ...
        */
        ...
        let annotations = jsonData.filter(d => d.PRESIDENTE == president );
      ...
      function redrawSmallMultiples() {
        ...
        if (annotate) {
          ...
          annotateChart(jsonData, target, presidents[1], chartScales)
